...
export function CartPage({ cart, onNavigate, onUpdateCart, onRemoveFromCart, currentUser }: CartPageProps) {
  const [selectedItems, setSelectedItems] = useState<number[]>(cart.map((_, i) => i));
    ...
    return (
      <div className="bg-gray-50 min-h-screen">
        <div className="max-w-[1280px] mx-auto px-6 md:px-8 py-20 text-center">
          ...
          <p className="text-sm text-gray-600 mb-6">원하시는 상품을 담아보세요!</p>
          <Button
            ...
            <div className="bg-white border border-gray-200 p-4 mb-4">
              <div className="flex items-center gap-2 mb-4 text-sm">
                ...
                <Button
                  ...
                  onClick={() => {
                    const itemsToRemove = [...selectedItems].sort((a, b) => b - a);
              ...
              <div className="space-y-4">
                {cart.map((item, index) => (
                  <div key={index} className="flex gap-4 p-4 border border-gray-200">
                    <Checkbox
                    ...
                    <div className="flex-1">
                      ...
                      {item.selectedColor && (
                        <p className="text-xs text-gray-500">색상: {item.selectedColor}</p>
                      )}
                      {item.selectedSize && (
                        <p className="text-xs text-gray-500">사이즈: {item.selectedSize}</p>
                      )}
                      <div className="flex items-center gap-3 mt-2">
                        <div className="flex items-center border border-gray-300">
                          <Button
                            ...
                            onClick={() => onUpdateCart(index, Math.max(1, item.quantity - 1))}
                          >
                          ...
                          <Button
                            ...
                            size="sm"
                            className="h-7 w-7 p-0"
                            onClick={() => onUpdateCart(index, item.quantity + 1)}
                          >
              ...
              <div className="space-y-3 mb-4 text-sm">
                ...
                {deliveryFee > 0 && (
                  <p className="text-xs text-gray-900">
                    {(30000 - selectedTotal).toLocaleString()}원 더 구매 시 무료배송
                  </p>
                )}
                <Separator />
