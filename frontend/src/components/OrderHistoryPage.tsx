import { Package, Truck, CheckCircle, XCircle, RotateCcw, ChevronRight } from 'lucide-react';
...
export function OrderHistoryPage({ currentUser, onNavigate }: OrderHistoryPageProps) {
  if (!currentUser) {
    return (
      ...
      <div className="max-w-[1280px] mx-auto px-6 md:px-8 py-6">
        ...
        <Tabs defaultValue="all">
          <TabsList className="mb-6 bg-white border border-gray-200">
            <TabsTrigger value="all" className="text-sm">전체 ({mockOrders.length})</TabsTrigger>
          ...
          <TabsContent value="all">
            {mockOrders.length === 0 ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                ...
                <p className="text-sm text-gray-600 mb-4">주문 내역이 없습니다</p>
                <Button
          ...
          <TabsContent value="preparing">
            {filterOrders('배송 준비 중').length === 0 ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                <p className="text-sm text-gray-600">배송 준비 중인 주문이 없습니다</p>
              </div>
            ) : (
              filterOrders('배송 준비 중').map(order => <OrderCard key={order.id} order={order} />)
            )}
          </TabsContent>
          ...
          <TabsContent value="shipping">
            {filterOrders('배송 중').length === 0 ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                <p className="text-sm text-gray-600">배송 중인 주문이 없습니다</p>
              </div>
            ) : (
              filterOrders('배송 중').map(order => <OrderCard key={order.id} order={order} />)
            )}
          </TabsContent>
          ...
          <TabsContent value="delivered">
            {filterOrders('배송 완료').length === 0 ? (
              <div className="bg-white border border-gray-200 p-12 text-center">
                <p className="text-sm text-gray-600">배송 완료된 주문이 없습니다</p>
              </div>
            ) : (
              filterOrders('배송 완료').map(order => <OrderCard key={order.id} order={order} />)
            )}
          </TabsContent>
        </Tabs>
      </div>
